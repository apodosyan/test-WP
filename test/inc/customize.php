...
function test_customize_register( $wp_customize ) {
	$color_scheme = test_get_color_scheme();
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'test_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'test' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => test_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'test' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'test' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'test' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'test' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'test' );
}
add_action( 'customize_register', 'test_customize_register', 11 );

/**
 * Render the site title for the selective refresh partial.
 *
 * @since Twenty Fifteen 1.5
 * @see test_customize_register()
 *
 * @return void
 */
 ...
 *
 * Can be filtered with {@see 'test_color_schemes'}.
 *
 ...
 */
function test_get_color_schemes() {
	/**
	 ...
	 * }
	 */
	return apply_filters( 'test_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'test' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'test_get_color_scheme' ) ) :
/**
 ...
 */
function test_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = test_get_color_schemes();
...
endif; // test_get_color_scheme

if ( ! function_exists( 'test_get_color_scheme_choices' ) ) :
/**
 ...
 */
function test_get_color_scheme_choices() {
	$color_schemes                = test_get_color_schemes();
...
endif; // test_get_color_scheme_choices

if ( ! function_exists( 'test_sanitize_color_scheme' ) ) :
/**
 ...
 */
function test_sanitize_color_scheme( $value ) {
	$color_schemes = test_get_color_scheme_choices();
...
endif; // test_sanitize_color_scheme
 ...
 */
function test_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = test_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = test_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = test_get_color_scheme_css( $colors );

	wp_add_inline_style( 'test-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'test_color_scheme_css' );
 ...
 * @since Twenty Fifteen 1.0
 */
function test_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', test_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'test_customize_control_js' );
...
function test_customize_preview_js() {
	wp_enqueue_script( 'test-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
 ...
 */
function test_get_color_scheme_css( $colors ) {
 ...
 * @since Twenty Fifteen 1.0
 */
function test_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-test-color-scheme">
		<?php echo test_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'test_color_scheme_css_template' );
